...
def boost_t_dep(h,c, hdot=None):    
    ...
    boost_t_dep = np.zeros((hdot.n_times, 3), dtype=float)
...
P_dot = scri.momentum_flux(h) 
L_dot = scri.angular_momentum_flux(h)
B_dot = scri.boost_flux(h) + boost_t_dep(h, -0.001)
